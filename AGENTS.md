...
This is `rhoai-brewet`, a community plugin for the **Red Hat OpenShift AI (RHOAI) Dashboard**. It uses Webpack 5 Module Federation to expose remote modules that the RHOAI dashboard host application loads at runtime.
...
- **`./extensions`** (`src/rhoai/extensions.ts`) — Defines seven extension points:
  - `app.area` — registers the `rhoai-brewet` feature area
  - `app.navigation/section` (x2) — `community-plugins` shared parent section (with `CommunityNavIcon`) and `rhoai-brewet` plugin subsection (with `BrewetNavIcon`)
  - `app.navigation/href` (x3) — "User Info", "Cluster Resources", and "Namespace Summary" nav items under the `rhoai-brewet` section
  - `app.route` — mounts the App component with wildcard routing at `/rhoai-brewet/*`
- **`./Icon`** (`src/app/components/BrewetNavIcon.tsx`) — SVG icon for the plugin's nav subsection. A separate `CommunityNavIcon.tsx` provides the icon for the shared `community-plugins` parent section.
...
The plugin has three pages, routed under `/rhoai-brewet/*`:

- **User Info page** (`src/app/pages/UserInfoPage.tsx`) — Displays the authenticated user's information via `/api/status`.
- **Cluster Resources page** (`src/app/pages/ClusterResourcesPage.tsx`) — Create and list Deployments and Services via the dashboard's K8s API pass-through (`/api/k8s/*`).
- **Namespace Summary page** (`src/app/pages/NamespaceSummaryPage.tsx`) — Displays aggregated namespace and pod data via the plugin's BFF service.
...
The `bff/` directory contains a standalone Express.js + TypeScript backend service. The dashboard proxies requests from `/rhoai-brewet/api/*` to this service, forwarding the user's Bearer token. See `docs/architecture/BFF_PATTERN.md` for details.
...
- `config/webpack.dev.js` — Dev server on port 9500, proxies `/rhoai-brewet/api` to BFF at `localhost:3000` and `/rhoai-brewet` to dashboard at `localhost:8443`
...
- **Helm chart**: `chart/` deploys to Kubernetes with Deployment + Service for both frontend and BFF. Frontend defaults to `quay.io/OWNER/rhoai-brewet:latest`, BFF to `quay.io/OWNER/brewet-bff:latest`.
